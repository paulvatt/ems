...
def add_emp(request):
    if request.method =='POST':
        ...
        new_emp.save()
        return HttpResponse('New Employee added - success')
    elif request.method == 'GET':
        return render(request,'add_emp.html')
    else:
        return HttpResponse("an Exemption occured!")
...
def remove_emp(request, emp_id = 0):
    if emp_id:
        try:
            ...
            return HttpResponse("Selected Employee deleted - success")
        except:
...
def filter_emp(request):
    if request.method =='POST':
        ...
        return HttpResponse('An exmption has occurred')
